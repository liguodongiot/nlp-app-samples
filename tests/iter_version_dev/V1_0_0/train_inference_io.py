...
from tests.iter_version_dev.V1_0_0.common import match_strategy
...
# 默认训练参数
def default_distil_train_args(output_dir):
    training_args = TrainingConfig(output_dir=output_dir)
    training_args.learning_rate = 1e-4
    training_args.num_train_epochs = 40
    training_args.per_device_train_batch_size = 32
    training_args.per_device_dev_batch_size = 32
    training_args.fp16 = False
    training_args.fp16_opt_level = "O1"
    training_args.device = torch.device(
        "cuda" if torch.cuda.is_available() else "cpu")
    training_args.weight_decay_rate = 0.01
    return training_args



# 默认蒸馏参数
def default_distil_config_args(distil_type):
    intermediate_matches = match_strategy[distil_type]
    distilling_args = DistillationConfig(intermediate_matches=intermediate_matches)
    distilling_args.temperature = 8
    distilling_args.hard_label_weight = 0
    distilling_args.kd_loss_type = 'ce'
    distilling_args.probability_shift = False
    return distilling_args
